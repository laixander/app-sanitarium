...
<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col font-sans overflow-hidden">
        <!-- Header -->
        ...
        <main class="flex-1 flex overflow-hidden">
            <!-- Now Serving -->
            <section class="flex-1 p-8 flex flex-col bg-gray-50/50 dark:bg-gray-950/50">
                ...
                <div class="flex-1 pr-4">
                    <div v-if="servingTickets.length === 0"
                        class="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-600">
                    ...
                    <TransitionGroup name="list" tag="div" class="grid grid-cols-1 xl:grid-cols-2 gap-6 relative">
                        <UCard v-for="ticket in servingTickets" :key="ticket.id"
                            :ui="{ body: 'flex justify-between items-center p-6 sm:p-12' }"
                            class="rounded-3xl shadow-xl transform transition-all duration-500 hover:scale-[1.02]">
                            <div class="flex flex-col gap-2 relative z-10 w-full">
                                <p
                                    class="text-2xl text-muted font-semibold uppercase tracking-wider flex items-center gap-2">
                                    <UIcon name="i-lucide-monitor" class="size-8" />
                            ...
                            <div class="flex flex-col gap-3 items-end relative z-10 shrink-0">
                                <UBadge v-if="ticket.isPriority" label="Priority" color="amber" icon="i-lucide-heart"
                                    class="rounded-full uppercase text-base font-semibold tracking-wider px-6 py-3"
            ...
            <aside
                class="w-[450px] border-l border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex flex-col z-10 shadow-2xl shrink-0">
                <div class="p-8 border-b border-default bg-muted backdrop-blur shrink-0">
                    <h2 class="text-xl font-extrabold flex items-center gap-3 tracking-tight uppercase">
