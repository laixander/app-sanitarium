...
<template>
    <UCard :ui="{ body: 'flex items-center justify-between sm:p-4' }"
        ...
        <UButton label="Call Again" icon="i-lucide-phone-forwarded" size="sm" color="error" :disabled="disabled"
            @click.stop="emit('recall')" />
    </UCard>
    <!-- <UCard :ui="{ body: 'flex items-center justify-between sm:p-4' }"
        class="rounded-xl shadow-sm border-l-4 border-red-500/50">
        <div class="flex items-center gap-3">
            <div
                class="flex flex-col items-center justify-center min-w-[48px] py-1 bg-muted rounded-lg border border-red-500/30">
                <UIcon name="i-lucide-user-x" class="size-4 text-red-500 mb-1" />
                <span class="font-extrabold text-lg leading-none text-red-600 dark:text-red-400">{{ ticket }}</span>
            </div>
        ...
        <UButton label="Call Again" icon="i-lucide-phone-forwarded" size="sm" color="error" variant="soft"
            @click="$emit('recall')" />
    </UCard> -->
</template>
